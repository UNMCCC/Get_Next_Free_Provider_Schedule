...
BEGIN
    ...
    -- #numbers is just integers from 0 to 49999 in this case
    -- here we are just being obtuse about creating a sequence is all.
    -- Monthly branches (16, 32): n <= 60  ->  n <= 600  (covers 50 years)
    -- Annual branches (64, 128): n <= 5   ->  n <= 50    (covers 50 years)
    --------------------------------------------------------------------
    IF OBJECT_ID('tempdb..#Numbers') IS NOT NULL DROP TABLE #Numbers;
    -- BugFix  #Numbers: 2000 -> 50,000 rows (covers ~137 years of daily offsets)
    SELECT TOP (50000)
    ...
    INTO #TemplBase
    FROM [hsc-cc-mqdb28].[Mosaiq].[dbo].[SchTempl] st WITH (NOLOCK) -- move to replica
    WHERE st.Disabled = 0
    ...
    SELECT
        ...
        Block_Type = CASE
			...
			WHEN NormActivity LIKE '%LAST DAY Dr. MAZ%'    THEN 'BLOCKING' -- maz
			WHEN NormActivity LIKE '%None clinic hours%'   THEN 'BLOCKING' -- 
			WHEN NormActivity LIKE '%Rounding%'            THEN 'BLOCKING' -- Ebaid's rounds
			WHEN NormActivity LIKE '%Fero Covering BMT%'   THEN 'BLOCKING' -- Fero busy with transplant
            WHEN NormActivity LIKE '%Tawfik grand Rounds%' THEN 'BLOCKING' -- Tawfik attending grand rounds fri 8am
